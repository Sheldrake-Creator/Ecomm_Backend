...
public class ReviewController {
        ...
        public ResponseEntity<HttpResponse> createReviewReview(@RequestBody ReviewRequest req,
                ...
                try {
                        ...
                        return ResponseEntity.status(HttpStatus.CREATED).body(HttpResponse.builder()
                                        .timeStamp(LocalDateTime.now().toString()).data(Map.of("review", review))
                                        .message("Review created successfully").status(HttpStatus.CREATED)
                                        .statusCode(HttpStatus.CREATED.value()).build());
                } catch (UserServiceException | ProductServiceException e) {
                        logger.error("Error creating review", e);
                        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                        .body(HttpResponse.builder().timeStamp(LocalDateTime.now().toString())
                                                        .message(e.getMessage()).status(HttpStatus.BAD_REQUEST)
        ...
        public ResponseEntity<HttpResponse> getProductReviews(@PathVariable Long productId)
                        throws ProductServiceException, UserServiceException {
                try {
                        ...
                        return ResponseEntity.ok(HttpResponse.builder().timeStamp(LocalDateTime.now().toString())
                                        .data(Map.of("reviews", reviews)).message("Reviews fetched successfully")
